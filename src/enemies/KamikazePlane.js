...
//  - flyby  (~42%): keeps buzzing past the player, looping back for more passes.
//  - loiter (~40%): circles above the player. Both stay near until shot down.
...
export class KamikazePlane extends Aircraft {
  constructor(scene, effects) {
    ...
    this.locked = false;
    this.maxAge = 120; // persist (loop/circle) rather than time out quickly
    ...
    this.flybyAlt = 12 + Math.random() * 12;
  }
  ...
  _behavior(dt, ctx) {
    ...
    if (this.state === 'flyby') {
      // Buzz the player and keep looping back for more passes (stays near).
      this.waypoint.set(
        player.position.x,
        player.position.y + this.flybyAlt,
        player.position.z
      );
    ...
    if (this.state === 'loiter') {
      // Circle above the player indefinitely.
